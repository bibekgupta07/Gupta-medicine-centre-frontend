...
import { Send, MessageSquare, Phone, Video, Wifi, WifiOff } from "lucide-react";
...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    const handleTyping = (data: unknown) => {
      ...
      setTypingUsers((prev) => [...new Set([...prev, d.user_name])]);
      setTimeout(() => {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 text-sm">
          {connected ? (
            <><Wifi size={16} className="text-green-600" /><span className="text-green-600">Connected</span></>
          ) : (
            <><WifiOff size={16} className="text-red-500" /><span className="text-red-500">Disconnected</span></>
          )}
        </div>
      </div>
          ...
          {loading ? (
            ...
            rooms.map((room) => (
              <button
                ...
                className={`w-full text-left p-4 border-b border-gray-100 hover:bg-gray-50 transition-colors ${selectedRoom?.id === room.id ? "bg-primary-50" : ""}`}
              >
                <div className="flex justify-between items-start">
                  <p className="font-medium text-gray-900 text-sm">{room.customer_name}</p>
                  {room.unread_count > 0 && (
                    <span className="bg-primary-600 text-white text-xs rounded-full px-2 py-0.5">{room.unread_count}</span>
                  )}
                </div>
                {room.last_message && <p className="text-xs text-gray-500 mt-1 truncate">{room.last_message}</p>}
              </button>
            ))
          )}
        </div>
        ...
        <div className="flex-1 flex flex-col">
          {selectedRoom ? (
              ...
              <div className="p-4 border-b border-gray-200 bg-white flex items-center justify-between">
                <div>
                  ...
                  {typingUsers.length > 0 && (
                    <p className="text-xs text-primary-600 animate-pulse">{typingUsers.join(", ")} typing...</p>
                  )}
                </div>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-3">
                {messages.map((msg) => (
                  <div key={msg.id} className={`flex ${msg.sender_role === "admin" ? "justify-end" : "justify-start"}`}>
                    <div className={`max-w-[70%] rounded-lg px-4 py-2 ${msg.sender_role === "admin" ? "bg-primary-600 text-white" : "bg-gray-100 text-gray-900"}`}>
                      {msg.message_type === "image" && msg.file_url && (
                        <img src={`${process.env.NEXT_PUBLIC_API_URL}${msg.file_url}`} alt="Shared" className="max-w-full rounded mb-1" />
                      )}
                      {msg.content && <p className="text-sm">{msg.content}</p>}
                      <p className={`text-[10px] mt-1 ${msg.sender_role === "admin" ? "text-primary-100" : "text-gray-400"}`}>
            ...
            <div className="flex-1 flex items-center justify-center text-gray-400">
              <div className="text-center"><MessageSquare size={48} className="mx-auto mb-3" /><p>Select a conversation</p></div>
            </div>
          )}
        </div>
      </div>
      ...
      <CallModal
        ...
        onClose={() => { setShowCallModal(false); setIncomingCall(null); setOutgoingUserId(""); }}
